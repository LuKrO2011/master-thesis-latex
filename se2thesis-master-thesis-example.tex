...
%	\lstinputlisting[language=Java, firstline=8, lastline=29, caption={The same example modified to be poorly readable},label={lst:cassandra-src-java-org-apache-cassandra-utils-modified}]{cassandra-src-java-org-apache-cassandra-utils-modified.java}
	...
	\begin{enumerate}
		\item \label{embedding-spaces-component} \textbf{(embedding-spaces-component)} We want to embed spaces and tabs as semantic tokens.
		\item \label{name-classifier-component} \textbf{(name-classifier-component)} We want to add a method name fitting classifier as a component of the overall model.
	\end{enumerate}
