...
\subsubsection{Prolific Study} \label{Prolific Study Results}
	In this section we summarize the results of the main study conducted via prolific.
	
\textit{1. Experimental setup:}

	The survey was conducted from the 31th January 2024 to the 7th February 2024. 221 participants were in total involved. Each of the 20 survey sheets was answered by 11 participants. For one survey prolific accidentally assigned one participant more. Each survey was configured so that a participant for one survey could not attend another one. Each survey had the same texts and configuration (see Appendix \ref{TODO}).
	
	You can find an example of a snippet being rated with the tool in figure \ref{fig:survey_tool}
	...
	% Survey
	\textit{1.2 Survey construction:}
	The first part of the survey is devoted to questionnaire attributes. The second part of the survey consists of 20 code snippets to be evaluated by the participants.
	In total, we create 20 unique questionnaires, with each questionnaire being rated by 10 individual participants. Consequently, we generate a dataset of 400 code snippets. The expected expenditure for this survey, involving 200 participants via the Prolific platform, was about €500.
	% Sample size
	To get accurate study results, multiple participants have to rate the same code snippets. We specify a size of 11 participants per code snippet, similar to \citeauthor{scalabrino2018comprehensive}~\cite{scalabrino2018comprehensive}. According to an online calculator\footnote{https://www.calculator.net/sample-size-calculator.html} we end up with a margin of error of 29.55\% at a confidence of 95\%. This means that we are 95\% confident that the actual readability of a code snippet is within 29.55\% of the survey result score. With a probability of 5\%, the readability of a code snippet differs by more than 29.55\% from the score of the survey result. On a Likert scale ranging from 1 to 5, the mean of the survey results is between 1.0 and 5.0. In this context, an offset of 29.55\% is an offset of more than one rating point~\cite{likert1932technique}. We need to consider this when evaluating our the gathered data.
	
	% Time Requirement
	\textit{1.3 Time requirement:}
	...
	A big part of our code snippets might be getters and setters. However, we do not want to mostly evaluate getters and setters in our study. Therefore, we use stratified sampling~\cite{thompson2012sampling} and craft the surveys as described in section \ref{Survey}.
			
	% Internal/Research Questions
	\textit{1.5 Sampling design:}
	...
	\textit{1.6 Questionnaire Attributes:}
	...
	\textit{1.8 Survey Result Evaluation:}
