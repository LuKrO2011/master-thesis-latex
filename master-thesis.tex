...
%	TODO
%	Prinzipiell schöne Übersicht von hier bis 2.5
%	Aber fehlt Einordnung: wie related das zu deinem Zeug? Z.B. be dem augmentation Paper kam meines Wissens ja raus, dass das die Modell-Ergebnisse ja tatsächlich verbessert.
%	Man könnte also z.B. schreiben: wir machen auch augmentation, weil das gut funktioniert, aber verfolgen einen Ansatz der ohne Domänenwissen auskommt.
...
% TODO
%	Ich würde den Absatz hierzu etwas relativieren.
...
%	Im Wesentlichen wird nur ein Knoten dahinter geschaltet, der abhängig vom Wert vorher dann eine Gruppe selektiert (</>=0.5).
%	Das, was du ansprichst, zielt ja eher darauf ab, dass für die Evaluierung die binäre Klassifizierung verwendet wird.
%	Und der Grund dafür ist sicherlich, dass es einfacher ist, das dazustellen, und dass man bessere Ergebnisse bekommt (das wissen denk ich auch alle, dafür brauchts nicht probably eigene research) :-) 
%	Ich würd also  nur schreiben:
%	- die Daten werden üblicherweise auf ner 5-point Likert scale erhoben
%	- am Schluss wird als binäre Klassifizierung evaluiert
%	- die Frage ist, wie dazwischen (beim Modelltraining) mit den Daten umgegangen wird. Mi machen das folgendermaßen... 
	...
	While the amount of classes varies, one can encode the data internally as a single continuous value between 0 and 1 where a higher value means higher readability. The output of the model is well readable, if the value after the last layer is above 0.5 and badly readable otherwise.
	
	Our evaluation model is the towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} and follows this encoding. We want to show how they transformed the rating scores into a binary classification problem. First, the mean values of all scores are calculated. In a second step, the snippets are ranked according to their mean score. Then, the top 25~\% of the data is labeled as well readable and the bottom 25~\% is labeled as badly readable. The 50~\% of the data in between is not used at all~\cite{mi2022towards}.
...
\subsection{Dataset Generation Approach} \label{Dataset Generation Approach}
	...
	While we refer to the merged data set (\Cref{Work on Existing Datasets}) as the merged data set, the data set generated by our approach is referred to as the new data set.
	...
	The fourth and final step is to generate badly readable code from the well readable one. Therefore we use the proposed Readability Decreasing Heuristics (see \Cref{Readability Decreasing Heursitcs}). Afterwards we again extract all methods with a comment at the beginning of the method.
...
%	\end{figure}
	...
	The individual methods are then extracted from the class files. As mentioned (see \Cref{Dataset Generation Approach}), we require a method comment for all methods. We therefore use the removeComment rdh after completing the method extraction.
...
\section{Evaluation} \label{Evaluation}
	...
	It was shown that Deep Learning models get better the more training data is available~\cite{hestness2017deep}. This holds under the assumption that the quality of the data is the same or at least similar. We want to check if the quality of our new data is sufficient for improving the deep learning-based readability classifier of \citeauthor{mi2022towards}~\cite{mi2022towards}. Therefore we will train their proposed model with combinations of the merged and the new dataset and compare the evaluation statistics.

\subsection{Survey} \label{Survey}
	The results of our survey are divided into two parts: The results of the pilot survey (see \Cref{Pilot Survey}), which were used to improve the main survey pre-launch, and the results of the main survey (see \Cref{Prolific Survey}), which were used to answer our research questions (\hyperref[select-well]{RQ1} and \hyperref[generate-poor]{RQ2}) and to craft our dataset.
...
\subsection{Model Training Results} \label{Model training results}
	...
	\begin{table}[tb]
		\centering
		\caption{Performance of different dataset configurations for the same model. New-Merged is training on the new dataset and fine tuning on the merged one.}
		\vspace{8pt}
