...
%	In the domain of software development, the significance of code readability cannot be overstated. 
	...
	In a second step, all selected Java files are manipulated so that the code is subsequently less readable. You can find an exemplary result of this in \autoref{lst:example-method}.
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	...
	Therefore, we applied stratified sampling, created specific configurations and selected the methods for our survey on this basis. An overview of the approach can be found in \autoref{fig:survey_pipeline}.
	...
	Since we want to compare the original methods with their modified variants later, we performed the sampling only for the original methods and added the \rdh methods in a later step.

	We first calculated features for the original code snippets. This was done using the tool of \citeauthor{scalabrino2018comprehensive}~\cite{scalabrino2018comprehensive}. We calculated a 110-dimensional feature vector for each original code snippet. Next we computed the cosine similarity metrix between all feature vectors using scikit\curl{https://scikit-learn.org/stable/modules/generated/sklearn.metrics.pairwise.cosine_similarity.html}{2024-02-20}. 
	Finally, using the fastcluster implementation~\cite{mullner2013fastcluster} of Ward's hierarchical clustering we were able to cluster the methods into an arbitrary amount of strata.	
...
%	The participants' familiarity with Java is shown in \autoref{fig:survey_Java_familiarity_pie}. According to their own estimation, 44.8~\% of the participants are experts with Java. Another 44.8~\% of the participants are either advanced or intermediate. This high level of familiarity with Java suggests that the quality of the evaluations received is also high.
	...
	In line with our expectation, we see that the ratings for \nonet and \texttt{methods} only differ by a very small amount of 0.02. A Mann-Whitney U test showed that the probability, that the difference between \nonet and \texttt{methods} is due to random variation, is 92~\%.
%	There is no significant difference between the two, as a Mann-Whitney U test confirmed. The probability that the difference between \nonet and \texttt{methods} is due to random variation is 92~\%. 
	...
	\autoref{fig:survey_ratings_bar_all} shows the differences in mean values more precise. \rdh reduced the readability for the configurations all7, commentsRemove, newlinesInsteadOfSpace, newlinesFew and tabs the most. The tool also reduced readability for newlinesMany, rename and spacesMany, but not as much.
