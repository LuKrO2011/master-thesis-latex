...
\subsection{Model Training Results} \label{Model training results}
	...
	\begin{enumerate}
		\item \textbf{Internal evaluation:} We investigate whether the towards model captures the structure of the mined-and-modified dataset and the difference between \texttt{original} and \texttt{all7} methods, which are modified with our RDHs.
		\item \textbf{Cross evaluation:} We investigate whether the mined-and-modified dataset is suited for predicting readability. We therefore train the model on the mined-and-modified dataset and evaluate it on the merged dataset.
		\item \textbf{Implementation evaluation:} We investigate whether our re-implementation of the towards, with the changes as described in \autoref{Readability Classification Model}, is able to achieve similar accuracy as the original one~\cite{mi2022towards}.
	\end{enumerate}
	
	We therefore used different combinations of training and evaluation datasets with the towards mode. The results can be found in \autoref{tab:dataset_performance}.
	...
	The towards model architecture is well suited to learn the structure of the mined-and-modified dataset. In particular it learns the differences between \texttt{original} and \texttt{all7} methods and thus it learns how to predict if a code snippet has been modified using RDHs. However, the readability determined with the merged dataset differs from the readability with the mined-and-modified dataset. Otherwise, the values for all-krod and krod-all would be similar to the value for all-all. This indicates that our dataset is not suitable for a general classification of readability, but we may have found a subproblem. However, adding more features to reduce readability and well-designed data augmentation could overcome this limitation.
