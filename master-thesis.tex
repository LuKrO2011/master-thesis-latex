...
%	% The towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} achieves an accuracy of 91.8\% when trained and evaluated on the new dataset. 
	...
	% Evaluation (short)
	We conducted a user study to validate our assumptions (Assumption \ref{well-readable-assumption} and \ref{poorly-readable-assumption}) and to thereby verify our new dataset.
...
\subsection{Readability Decreasing Heuristics} \label{Readability Decreasing Heursitcs}
	...
	These modifications are performed on the AST representation of the code to ensure that the functionality stays the same:
	\begin{itemize}
	...
	These modifications are performed on source code during pretty-printing as they cannot be displayed in the AST (see \autoref{tab:rdhs}):
	\begin{itemize}
...
%	\end{sloppypar}
	...
	While \texttt{starImport} might have an impact on the readability of class files it has none on methods as in Java the import statement are not within the methods in Java. As we end up with extracting methods for our dataset, \texttt{starImport} has no impact.
