...
\subsection{REDEC: Readability Decreaser} \label{REDEC}
	...
	\rdh uses a set of code modification heuristics that are applied to Java files. We call these \RDMs.
		...
		\begin{tabular}{|p{0.03\textwidth}|p{0.24\textwidth}|p{0.43\textwidth}|p{0.125\textwidth}|}
			...
			\hline
			9 & \texttt{renameVariable} & Renaming a variable declaration and its usages & \autoref{lst:java-method-poor}, Line 1-3 \\
			\hline
			...
			\hline
			12 & \texttt{inlineMethod} & Inline a method into its usages & \autoref{lst:java-class-file-poor}, Line 7-8 \\
			\hline
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	...
	\textbf{Stratified sampling:} Our first goal was to distinguish between very simple methods such as Getters and Setters and more complex method. We therefore applied stratified sampling~\cite{thompson2012sampling} to distinguish between very simple methods such as Getters and Setters and more complex methods. Since we want to compare the original methods with their modified variants later, we performed the sampling for the original methods and added the \rdh methods in a later step.
	...
	\textbf{Configuration selection:} The next step was to find realistic configurations for the \rdh tool. We selected first configurations by manually checking individual outputs of \rdh. Then we conducted a pilot study and adjusted the \mods based on the feedback.
...
%		\end{subfigure}
	...
	In a first step we use Imgkit\curl{https://pypi.org/project/imgkit/}{2024-03-02} to convert the code to HTML. Thereby, a HTML class is assigned to each type of syntactic element. Next, we apply syntax highlighting using a CSS style sheet (see \Cref{appendix:model-visual-colors}). In a third step we use pygments\curl{https://pygments.org/}{2024-03-02} to convert the HTML with the applied CSS to an image. We use pillow\curl{https://pypi.org/project/pillow/}{2024-03-02} to remove blur and adjust the padding of the image. Finally, the image is loaded using opencv-python\curl{https://pypi.org/project/opencv-python/}{2024-03-02} which allows us to convert the image to an RGB tensor that is suitable as a model input. This approach is fully automatized and which colors used for which elements can be eaily adjusted using the css stylesheet.
...
%	We are conducting a user study to determine the quality of the new dataset. In detail the aim of the user study is to answer the following key questions:
	...
	\begin{resq} \label[rq]{modify-poor} \textbf{(modify-poor)} Can poorly readable code be generated from well readable code?\end{resq} 
	...
	whether \rdh (see \Cref{REDEC}) is able to achieve this.
...
\subsection{Survey} \label{Survey}
	The results of our survey are divided into two parts: The results of the pilot survey, which were used to improve the main survey pre-launch, and the results of the main survey, which were used to answer our research questions (\Cref{mined-well} and \Cref{modify-poor}) and to craft our dataset.
...
%	\end{figure}
	...
	Thus, we re-examined the \rdh configurations and found that some of them are over-configured. This not only affects their readability, but also makes them look as if they were not written by human hands. We have therefore reduced the probabilities for these configurations.
...
%	We label each method in both groups with the corresponding mean score.
	...
	\begin{summary}{\hyperref[mined-well]{RQ 1 - mined-well}}
		...
		The mean score for the \texttt{original} methods of the mined-and-modified dataset (3.69) is significantly larger than the mean score for all ratings in the merged dataset (3.46). Therefore we reject the null hypothesis and conclude that well readable assumption (\Cref{well-readable-assumption}) holds.
	...
	We can be sure that the scores of all methods except \texttt{newlines\_many} and \texttt{rename} are indeed statistically different from the scores of \nonet. 
	
	If we consider binary readability classification and split the data into two classes (poorly readable: 1,2; well readable: 3-5) all but \texttt{rename} are statistically different from \nonet. This includes \texttt{newlines\_many} ($p=0.035=3.5~\% < 5~\%$) for which we could not confirm statistical difference without binary classification.
...
\section{Discussion} \label{Discussion}
	...
	% Mean assignment
	A drawback of our approach is that we rely on estimations to create the mined-and-modified dataset. The score labels of our code snippets are rough estimations and not exact values (see \Cref{Dataset Generation Approach}). Accurate ratings would require manual review of \numSamples code snippets by human annotators and is therefore not feasible. However, for two class classification this does not matter.
