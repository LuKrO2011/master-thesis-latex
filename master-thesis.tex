...
%	However, there are various definitions in the literature:
	...
	Considering these aspect, the definition of \citeauthor{oliveira2020evaluating} captures the meaning of code readability best. Therefore, we use their definition:
...
\subsection{Data Generation} \label{Data Generation}
	...
	In addition to related work on models and datasets, there is also related work which uses some of the ideas that we employ in our proposed approach for data generation. 
	...
	Another concept we employ is the intentionally degradation of source code to create a dataset.
	
	% Related work: Vulnerability insertion
	In the field of vulnerability detection of source code, \citeauthor{dolan2016lava} introduced a technique to automate the insertion of bugs into program source code. The generated dataset was used for vulnerability discovery tool evaluation~\cite{dolan2016lava}. 
	
%	% Related work: Vulnerability insertion
%	\citeauthor{dolan2016lava} introduced a technique designed to automate the insertion of bugs into program source code. The generated dataset is used for vulnerability discovery tool evaluation~\cite{dolan2016lava}.
%	
%	% Related work: Vulnerability insertion
%	\citeauthor{pewny2016evilcoder} introduced a system to introduce vulnerable source code to suited locations so that the code becomes exploitable~\cite{pewny2016evilcoder}.
	
	% Related work: Bug insertion
	\citeauthor{pradel2018deepbugs} apply a similar approach for the purpose of bug detection through a deep learning classifier. They generate training data through insertion of bugs into existing code. This process involves simple program transformations that introduce likely incorrect code examples, enabling the training of a classifier to distinguish between correct and incorrect code~\cite{pradel2018deepbugs}. \citeauthor{yasunaga2020graph} did similarly corrupt working programs into broken ones. They also suggest to pre-train models effectively before fine-tuning them on a manually labeled, target task~\cite{yasunaga2020graph}. We will employ the idea of pre and fine-tuning as well. \citeauthor{allamanis2021self} enhanced their approach by generating the dataset as a by product of co-training two models. One model is a detector that learns to detect and repair bugs in the code. The second model is a selector, that learns to create buggy code that is hard to detect~\cite{allamanis2021self}.
		
	% Related work: \rdh
	\citeauthor{loriot2022styler} took the idea and shifted the are to formatting. This topic comes closest to our topic of code readability. They created a model that is able to fix Checkstyle\curl{https://checkstyle.org/}{2023-07-25} violations using Deep Learning.
	In a first step, they inserted formatting violations into the code based on a project specific format checker ruleset.
	They then used a LSTM neural network that learned how to undo those injections. Their approach is working on abstract token sequences. Again, they generated the data in a self-supervised manner~\cite{loriot2022styler}. 
	
	While the approach of intentional denigrating of source code to create a dataset was applied to the fields of vulnerability detection, bug detection and code formatting, we are the first to apply it to the field of code readability.
	
	% TODO: Add co-training idea to future work
...
%	You can find a visualization in \autoref{fig:dataset_generation}.
	...
	Our approach is divided into four parts (\autoref{fig:dataset_generation}). We use the first three steps to mine well readable Java code. In a final step, we modify the well readable code to achieve our second goal, namely poorly readable source code.
	
	We start by querying the GitHub REST API\curl{https://docs.github.com/en/rest}{2024-02-15} for repositories that use checkstyle (query string: \textquote{checkstyle filename:pom.xml}). The repository informations (including the URL) are stored together with the main branches.
	...
	Finally, we take the mined well readable methods and the the modified poorly readable methods and combine them into the mined-and-modifed dataset. We remove code snippets that are identical for the original and the modified variant (\Cref{REDEC}) and balance the dataset using random sampling.
...
\subsection{REDEC: Readability Decreaser} \label{REDEC}
	...
	The second part is not encoded in the AST and is therefore executed at source code level immediately after the reverse transformation. For example, the AST does not encode line breaks, so that changes to these line breaks must be applied to the source code rather the AST. You can see which modifications are applied when in \autoref{tab:rdh-characteristics}.
...
%	In \autoref{tab:rdh-characteristics} we can see which modifications are applied when.
	...
	\begin{table}[tb]
		\centering
		\caption{Available \RDMs along with their execution type (on AST or Code), their configuration type (a array of probabilities or a single one) and whether they are included in the final dataset. \Cref{appendix:rdh-config-file} for a concrete configuration example.}
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	...
	For further investigation, we also calculate the difference of the merge distance to the merged distance of the previous step $x-1$:
	...
	\begin{figure}[tb]
		...
		\caption{Merge distances and difference to previous merge distance. The optimal difference to previous merge distance is highlighted in red.} 
...
%		\end{subfigure}
	...
	After extracting the features from the three individual representations the output is merged and used as an input for the final step: code readability classification. In this step, the model consists of multiple fully-connected layers and a dropout layer. The output is a single value, namely the readability score. If the score is above a certain threshold, we classify the input as well readable, otherwise it is poorly readable~\cite{mi2022towards}.
	...
	In a first step we use Imgkit\curl{https://pypi.org/project/imgkit/}{2024-03-02} to convert the code to HTML. Thereby, a HTML class is assigned to each type of syntactic element. Next, we apply syntax highlighting using a CSS style sheet (\Cref{appendix:model-visual-colors}). In a third step we use pygments\curl{https://pygments.org/}{2024-03-02} to convert the HTML with the applied CSS to an image. We use pillow\curl{https://pypi.org/project/pillow/}{2024-03-02} to remove blur and adjust the padding of the image. Finally, we load the image using opencv-python\curl{https://pypi.org/project/opencv-python/}{2024-03-02} which allows us to convert the image to an RGB tensor that is suitable as a model input.
...
%	We are conducting a user study to determine the quality of the new dataset. In detail the aim of the user study is to answer the following key questions:
	...
	It is not sufficient to have only well readable code for training a classifier. We also need poorly readable code. Therefore, we try to generate such code from the well readable code. We investigate whether this is possible in principle, and	whether \rdh (\Cref{REDEC}) is able to achieve this.
...
\textit{1. Experimental setup:}
	...
	The survey was conducted between January 31 and February 7 in 2024. A total of 221 participants took part. 11 participants  answered each of the 20 questionnaires (similar to the survey of \citeauthor{scalabrino2018comprehensive}~\cite{scalabrino2018comprehensive}). In one survey, we assigned one more participant by mistake. 
...
\section{Discussion} \label{Discussion}
	...
	The question arises what the different aspects are and whether it is possible to extend \rdh (\Cref{REDEC}) so that the same aspects are captured. By achieving this, we suggest that we could also achieve better evaluation results on the merged dataset than previous models.
	...
	When merging existing datasets \citeolddataset into a single dataset (\Cref{Work on Existing Datasets}), we set readability score of a code snippet as the mean value of all its ratings. As only a limited number of people participated in each survey, this may introduce errors due to statistical deviations.
