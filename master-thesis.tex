...
%	\addtocontents{toc}{\protect\setcounter{tocdepth}{0}} % Limit depth to section level
%	\addtocontents{toc}{\protect\contentsline{section}{\protect\numberline{4.4}Your Section Title}{}} % Insert section 4.4 into TOC
%	\addtocontents{toc}{\protect\setcounter{tocdepth}{2}} % Reset depth to default
	\tableofcontents
...
%		\item We show the effectiveness of our generation approach through a survey (\Cref{Prolific Survey}).
...
%	The survey confirms both assumptions (\Cref{well-readable-assumption} and \Cref{poorly-readable-assumption}) (\Cref{Prolific Survey}).
...
%	\end{sloppypar}
	...
	There is a special case regarding \texttt{removeComment}: \texttt{comments-remove} and \texttt{all7} use \texttt{removeComment} at 100~\% for the user survey (\Cref{Prolific Survey}). The reason for this is that a method without comments is realistic. For training and evaluation of the deep-learning model, the probability was set to 10~\% (for both affected configurations, \texttt{all7} and \texttt{removeComment}) to avoid shortcut learning (\Cref{Model training results}).
...
\section{Construction of Questionnaires} \label{Construction of Questionnaires}
	We evaluate the generated dataset and the new approach with a survey (\Cref{Prolific Survey}). However, we cannot evaluate all mined-and-modified methods as the original methods consist of \numOriginal samples and there is an infinite number of possible configurations for \rdh that can be applied to each method.
...
\subsection{Results}
	...
	\autoref{fig:survey_Java_familiarity_pie} shows the participants' familiarity with Java according to their estimation. The participants are therefore grouped into five familiarity-groups according to their answer options: \texttt{Novice}, \texttt{Beginner}, \texttt{Intermediate}, \texttt{Advanced} and \texttt{Expert}.
	...
	\autoref{fig:java_knowledge_time} shows the participants' time required of each Java familiarity-group to complete a questionnaire. While the median is similar for all groups, the distribution varies. In the \textit{Novice} group the completion times are most dense, followed by the \textit{Beginner} and the \textit{Expert} group. \textit{Novice} and \textit{Beginner} raters might not fully grasp what the presented code does and therefore be faster in completing the task as they don't go that much into detail. Some of the \textit{Experts} on the other hand might read the given code snippets faster than others and therefore require less time.
	While there are no outliers for \textit{Novice} and \textit{Beginner} there are outliers for \textit{Intermediate}, \textit{Advanced} and \textit{Expert}. The reasoning is the same.
	
	\autoref{fig:java_knowledge_ratings} shows the participants' rating deviations from the mean values of code snippets per Java familiarity-group. The rating deviation is normalized by summarizing the deviations for each rater for each group and then divided by the number of participants within each group. We can see that the deviations of all groups are very similar.
	We performed a statistical test, one-way ANOVA, and obtained a p-value of $0.40$. This is not smaller than $5\%=0.05$. The deviations are not significant.
	The similarity indicates that the quality of ratings of all familiarity-groups can be considered as equal. This is the case even though the time requirement distribution is not that equal (\autoref{fig:java_knowledge_time}).
	
	By analyzing various aspects of the Java familiarity-groups in \autoref{fig:java_familiarity} we suggest that the overall quality of the evaluations received is high.
...
%	We delve into important intricacies below.
