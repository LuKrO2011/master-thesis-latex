...
% TODO: poorly readable vs unreadable? Currently poorly readable
% TODO: Links über die ganzen wörter (e.g. appendix)
...
\section{Introduction} \label{Introduction}
	...
	We select Java files from the repositories that meet our criteria and extract methods from the Java classes in these files and label them as well readable (\Cref{well-readable-assumption}).
...
%	% The towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} achieves an accuracy of 91.8\% when trained and evaluated on the new dataset. 
	...
	% Evaluation (short)
	We conducted a user study to validate our assumptions (\Cref{well-readable-assumption} and \Cref{poorly-readable-assumption}) and to thereby verify the mined-and-modified dataset.
...
%	\begin{listing}[tb]
	...
	%		\begin{sublisting}{\linewidth}
		...
		%		\end{sublisting}
	%		\caption{Comparing code readability: well-readable code (\autoref{lst:cassandra-src-Java-org-apache-cassandra-utils}) vs. poorly readable modification (\autoref{lst:cassandra-src-Java-org-apache-cassandra-utils-modified}).}
	%	\end{listing}
...
%	\end{table}
	...
	\begin{table}[tb]
		\centering
		\caption{Available Readability Decreasing Heuristics along with when they are executed (on AST or Code), their configuration type (a array of probabilities or a single one) and whether they are included in the final dataset. See \Cref{appendix:rdh-config-file} for a concrete configuration.}
		\vspace{8pt}
		\label{tab:rdhs}
		\begin{tabular}{llllc}
			\toprule
...
%	\begin{table}[tb]
%		\centering
%		\caption{Available Readability Decreasing Heuristics. For heuristics $1-4$ (\texttt{newline}, \texttt{incTab}, \texttt{decTab} and \texttt{space}), several probabilities can be defined for the respective number of replacements (see \Cref{appendix:rdh-config-file}).}
%		\vspace{8pt}
%		\label{tab:rdhs}
%		\begin{tabular}{lllc}
%			\toprule
...
\section{Evaluation} \label{Evaluation}
	...
	\begin{enumerate}[label={Assumption \arabic*},ref={\arabic*},leftmargin=*]
		...
		\item \label[assumption]{poorly-readable-assumption} \textbf{(poorly-readable-assumption)} After applying Readability Decreasing Heuristics, the code is poorly readable.
	\end{enumerate}
