...
\newcommand{\RDHs}{Readability Decreasing Heuristics\xspace}
\newcommand{\rdh}{REDEC\xspace}
\newcommand{\rdhs}{RDHs\xspace}
\newcommand{\RDHa}{\RDH (\rdh)\xspace} % With abbrevation
\newcommand{\RDHsa}{\RDHs (\rdhs)\xspace} % With abbrevation
...
% TODO: Check times for background and realted work (states/stated, writes/wrote)
% TODO: Add figures as pdf or high quality, without internal captions (python)
% Poorly readable vs unreadable? Currently poorly readable

% TODO: While proof reading:
% TODO: Check tenses (Present tense to describe the paper itself) again?
% TODO: Clearify code snippets vs methods. Done?
% TODO: Check if RECED, RDH, mined-and-modified and merged dataset make sense and are consistent 
% TODO: Find where to add this:	Once the survey was completed, we aggregated the ratings across all strats and all methods. We grouped the results into the 10 \rdhs. In this way, we assess whether the \rdhs work, which \rdhs work best and how strong the impact of each \rdh on readability is. In addition, we use the results of the survey to label the mined-and-modified dataset, which consists of the original and all7 \rdh methods.
\begin{document}
...
%	In the domain of software development, the significance of code readability cannot be overstated. 
	...
	In a second step, all selected Java files are manipulated so that its code is subsequently less readable. You can find an exemplary result of this in \autoref{lst:example-method}.
	We extract methods from the Java classes in these files and label them as poorly readable (\Cref{poorly-readable-assumption}).
...
%	% The towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} achieves an accuracy of 91.8\% when trained and evaluated on the new dataset. 
	...
	\begin{itemize}
		...
		\item We develop an approach for mining well readable Java methods and thereby achieve automated dataset generation for code readability. This allows us to introduce a new dataset (\Cref{Dataset Generation Approach}).
		\item We managed to create a tool that can automatically decrease the readability of Java class files (\Cref{Readability Decreasing Heursitcs}).
		\item There is an enormous amount of possible combinations to sample methods for a user study. We show a representative and resource-effective sample approach (\Cref{Construction of Questionnaires}).
		\item We demonstrate that the model of \citeauthor{mi2022towards}~\cite{mi2022towards} is limited (\Cref{Readability Classification Model}).
		\item We show that our generation approach is working (\Cref{Survey}).
		\item We show that our dataset can be used to train code readability classification models by training and evaluating the performance of the model of \citeauthor{mi2022towards}~\cite{mi2022towards} with and without the mined-and-modified dataset (\Cref{Model training results}).
	\end{itemize}
...
%	Although our approach for creating a dataset works in principle, we are not able to address the same aspects of code readability as the merged dataset does with the proposed heuristics. Thus, our dataset probably only addresses a partial problem of code readability.
	...
	Our approach for creating a dataset works in principle. However, the model training results show (see \Cref{Model training results}) that the mined-and-modified dataset does not capture the same aspects of code readability as the merged dataset. 
...
\subsection{Dataset Generation Approach} \label{Dataset Generation Approach}
	...
	The fourth and final step is to generate poorly readable code from the well readable one. Therefore we use the proposed \RDHs (see \Cref{Readability Decreasing Heursitcs}). Afterwards we again extract all methods with a comment at the beginning of the method.
	...
	After removing code snippets that are identical for the original and the variant with reduced readability (see \Cref{Readability Decreasing Heursitcs}) and balancing the dataset using random sampling, the result is a dataset consisting of \numSamplesAccurate code snippets.
	
\subsection{\RDHs} \label{Readability Decreasing Heursitcs}
	In this section we explain how we achieved to decrease the readability of code using the \RDHa.
	\rdh uses a set of code manipulation heuristics that are applied to Java files. We call these \RDHsa.
	One part is performed on the Abstract Syntax Tree (AST) representation of the Java files using the spoon library\curl{https://spoon.gforge.inria.fr/}{2024-15-02}~\cite{pawlak2016spoon}. Another part is executed when pretty-printing the AST back into Java files. This part cannot be displayed in the AST and is therefore executed at source code level immediately after the reverse transformation.
	...
	\rdh initially converts the Java code of a well readable Java class file into an AST. In the end the AST is parsed back to Java code using an Pretty Printer. If nothing else is done, this results in \nonet. Note that the code of \nonet is slightly different from the original input code, as the styling and formatting of the original code will be overwritten by the default formatting of the Java Pretty Printer of the spoon library\curl{https://spoon.gforge.inria.fr/}{2024-15-02}~\cite{pawlak2016spoon}.
	...
	\begin{table}[p]
		...
		\begin{tabular}{|p{0.03\textwidth}|p{0.24\textwidth}|p{0.43\textwidth}|p{0.125\textwidth}|}
			...
			\hline
			15 & \texttt{insertBraces} & Inserting superfluous braces & \autoref{lst:java-method-poor}, Lines 3-4 \\
			\hline
			16 & \texttt{starImport} & Replacing specific imports with star-import & \autoref{lst:java-class-file-poor}, Line 1 \\
			\hline
			17 & \texttt{inlineField} & Inlining the values of static fields into the code & \autoref{lst:java-class-file-poor}, Line 7 \\
			\hline
			18 & \texttt{partially Evaluate} & Partially evaluate constants & \autoref{lst:java-class-file-poor}, Line 4 \\
			\hline
		\end{tabular}
	\end{table}
	...
	\begin{table}[tb]
		\centering
		\caption{Available \RDHs along with when they are executed (on AST or Code), their configuration type (a array of probabilities or a single one) and whether they are included in the final dataset. See \Cref{appendix:rdh-config-file} for a concrete configuration example.}
		\vspace{8pt}
...
%	\begin{table}[tb]
%		\centering
%		\caption{The \RDHs that are performed on the AST.}
%		\vspace{8pt}
%		\label{tab:ast-\rdhs}
...
%	\begin{table}[tb]
%		\centering
%		\caption{The \RDHs that are performed while pretty-printing}
%		\vspace{8pt}
%		\label{tab:code-\rdhs}
%		\begin{tabular}{|p{0.15\textwidth}|p{0.6\textwidth}|p{0.125\textwidth}|}
%			\hline
...
%	\end{sloppypar}
	...
	For heuristics of configuration type \textit{Single} (others) a single probability must be defined (see \autoref{tab:rdh-characteristics}).
	...
	%	\begin{table}[tb]
		%		\centering
		%		\caption{Chosen configurations and their probabilities for the \RDHs.}
		%		\vspace{8pt}
...
%	\begin{table}[tb]
%		\centering
%		\caption{\RDHs that are not included in the final version of the mined-and-modified dataset and why.}
%		\vspace{8pt}
...
%	\begin{table}[tb]
%		\centering
%		\caption{Available \RDHs. For heuristics $1-4$ (\texttt{newline}, \texttt{incTab}, \texttt{decTab} and \texttt{space}), several probabilities can be defined for the respective number of replacements (see \Cref{appendix:rdh-config-file}).}
%		\vspace{8pt}
%		\label{tab:\rdhs}
%		\begin{tabular}{lllc}
%			\toprule
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	We evaluated the generated dataset and the new approach with a survey. To do this, we had to carefully select suitable code snippets from the dataset. An overview of the approach can be found in \autoref{fig:survey_pipeline}.
...
%	It is not sufficient to have only well readable code for training a classifier. We also need poorly readable code. Therefore, we will try to generate such code from the well readable code. We will investigate whether this is possible in principle, and we will propose an automated approach for archiving this: \RDHs.
%	
...
%		\end{subfigure}
	...
	In a first step we use Imgkit\curl{https://pypi.org/project/imgkit/}{2024-03-02} to convert the code to HTML. Thereby, a HTML class is assigned to each type of syntactic element. Next, we apply syntax highlighting using a CSS style sheet (see \Cref{appendix:model-visual-colors}). In a third step we use pygments\curl{https://pygments.org/}{2024-03-02} to convert the HTML with the applied CSS to an image. We use pillow\curl{https://pypi.org/project/pillow/}{2024-03-02} to remove blur and adjust the padding of the image. Finally, the image is loaded using opencv-python\curl{https://pypi.org/project/opencv-python/}{2024-03-02} which allows us to convert the image to an RGB tensor that is suitable as a model input.	
	...
	\begin{listing}[tb]
		...
		\end{minted}
		\caption{The Java method from \autoref{lst:java-method-well} encoded and decoded using BERT-base-cased with a limit of 100 tokens. Space characters separate the tokens. Newlines are preserved for readability.}
		\label{lst:print-greeting}
	\end{listing}
	
	Consider the method from \autoref{lst:example-method-well}. With a token limit of 100, the last encoded token is the last closing parenthesis in line 9. Everything from line 10 onwards is not encoded, which means that the information is lost for the semantic part of the model. To put it in other words: The model of \citeauthor{mi2022towards} only considers the first few lines of code snippets in its semantic component.
...
\section{Evaluation} \label{Evaluation}
	...
	\begin{enumerate}[label={Assumption \arabic*},ref={\arabic*},leftmargin=*]
		...
		\item \label[assumption]{poorly-readable-assumption} \textbf{(poorly-readable-assumption)} After applying \RDHs, the code is poorly readable.
	\end{enumerate}
	...
	In our new approach for generating training data, we assume that the code from repositories is well readable under certain conditions (\Cref{well-readable-assumption}). We want to check whether that holds. To answer this question we will use the results of the user study.
	...
	As the name already suggests, the applied transformations on the source code are only heuristics. To answer, whether the generated code is poorly readable (\Cref{poorly-readable-assumption}) we will utilize the results of the user study.
...
\textit{1. Experimental setup:}
	...
	Each questionnaire consists of 20 code snippets. Consequently, 400 different code snippets are rated in total. The questionnaires were configured in a way that each participant could only take part in one of the questionnaires. You can find the texts for the survey in \Cref{appendix:prolific-survey-texts}. The questionnaires were crafted as described in \Cref{Construction of Questionnaires}.
...
%	\end{table}
	...
	\begin{itemize}
		...
		\item For \Cref{poorly-readable-assumption}: The readability of code does not significantly deteriorate compared to the original code snippet.
	\end{itemize}
...
\textit{2. Threats:}
	...
	\begin{itemize}
		...
		\item \textbf{Sampling Method:} We have a larger proportion of code snippets from strata 1 and 3 (see \Cref{Construction of Questionnaires}). While we argue that we avoided spending resources on labeling \rdh data that is likely not different from the original methods or rather uninteresting, this might also introduce statistical errors to our survey results.
...
%	We label each method in both groups with the corresponding mean score.
	...
	\begin{researchbox}{Summary (RQ1 - mined-well):}
		...
		The mean score for the \texttt{original} methods of the mined-and-modified dataset (3.68) is significantly larger than the mean score for all ratings in the merged dataset (3.45). Therefore we reject the null hypothesis and conclude that well readable assumption (\Cref{well-readable-assumption}) holds.
	...
	We analyzed whether the difference in ratings between the different \rdhs is statistically significant. To do this, we used the Mann-Whitney U test to compare the ratings for all snippets for an \rdh with the corresponding \nonet snippets. The results can be found in \autoref{tab:survey_statistical_evidence}.
	If we compare the \rdhs with the \nonet methods, we can be sure that the scores of all methods except newlines many and rename are indeed statistically different from the scores of \nonet. 
	
	If we consider binary readability classification and split the data into two classes (poorly readable: 1,2; well readable: 3-5) all but \nonet and \texttt{rename} are statistically different from each other. This includes \texttt{newlines many} (TODO: Add p Values) for which we could not confirm statistical difference without binary classification.

	Besides \nonet, this leaves only \texttt{rename} where we can not confirm statistical significance. Overall, we showed that the heuristics actually reduce the readability of source code.
...
%	\end{table}
	...
	\begin{table}[tb]
		\centering
		\caption{Mann-Whitney U test results of each \rdh against \nonet. When \textbf{$p$} is smaller than \num{5e-2} (not bold) we conclude that the difference is significant.}
		\vspace{8pt}
		\label{tab:survey_statistical_evidence}
		\begin{tabular}{lS[table-format=1.2e-2]}
			...
			newlines\_few & \num{5.23e-6} \\
			spaces\_many & \num{4.07e-2} \\
			\textbf{newlines\_many} & \num{3.00e-1} \\
			comments\_remove & \num{3.64e-3} \\
			\textbf{rename} & \num{9.90e-2} \\
			newline\_instead\_of\_space & \num{4.57e-6} \\
			tabs & \num{3.06e-8} \\
			all7 & \num{1.80e-7} \\
			\bottomrule
		\end{tabular}
...
\section{Discussion} \label{Discussion}
	...
	Our survey (see \Cref{Pilot Survey}) showed that mined-and-modified dataset captures readability. The model training results (see \Cref{Model training results}) showed that the mined-and-modified dataset captures different aspects of readability than the merged dataset. 	
	% Different aspects of readability
	The question arises what the different aspects are and whether it is possible to extend the proposed \rdhs (see \Cref{Readability Decreasing Heursitcs}) so that the same aspects are captured. By achieving this we suggest that we could also achieve better evaluation results on the merged dataset than previous models.
	
	% Model Restrictions
	We fine-tuned the model with the merged dataset after training with the mined-and-modified dataset and evaluated it on the merged dataset (see \Cref{Model training results}). We expected the classification accuracy of the resulting model to exceed that of the model trained on the merged dataset only.
	...
	% Model threats	
	When comparing the model performance trained on the mined-and-modified and merged dataset, it should be noted that the merged dataset is small. Consequently, comparisons between classifiers trained on the merged dataset may be unreliable~\cite{mi2022towards}.
...
\section{Conclusions} \label{Conclusions}
	...
	% Summary
	In summary, there are many opportunities to further investigate and thus most likely improve the classification of code readability. Our new dataset and the generation approach could be useful here.
