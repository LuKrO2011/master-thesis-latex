...
%	\end{table}
	...
	In a second step, we applied a stratified sampling~\cite{thompson2012sampling} to distinguish between very simple methods such as getter and setter and more complex methods. In order to be able to compare the original methods with their modified variants, we only carried out the random sampling for the original methods and compared the rdh methods with these in a later step.
...
\textit{1. Experimental setup:}
	The survey was conducted using Tien Duc Nguyen's Code Annotation Tool (see Figure \ref{fig:survey_tool}) along with the platform prolific\curl{https://app.prolific.com/}{2024-02-21} for the recruitment and payment of participants.
