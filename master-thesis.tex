...
\subsection{Data Generation} \label{Data Generation}
	...
	% Related work: \rdh
	\citeauthor{loriot2022styler} created a model that is able to fix Checkstyle\curl{https://checkstyle.org/}{2023-07-25} violations using Deep Learning.
	In a first step, they inserted formatting violations into the code based on a project specific format
	checker ruleset.
	They then used a LSTM neural network that learned how to undo those injections. Their approach is working on abstract token sequences. They generated the data in a self-supervised manner~\cite{loriot2022styler}. \citeauthor{yasunaga2020graph} explored a similar idea~\cite{yasunaga2020graph}. We use the idea of intentional degradation of code for data generation.
...
\subsection{Abstract Syntax Tree} \label{Abstract Syntax Tree}
	...
	We transform source code into the AST, make certain modifications to the AST (\Cref{tab:rdh-characteristics}) and then transform the AST back into the code representation. Compared to performing the modifications on the code, this has certain advantages, which are further detailed in \Cref{REDEC}.
...
%	You can find a visualization in \autoref{fig:dataset_generation}.
	...
	In a third step we run checkstyle\curl{https://checkstyle.sourceforge.io/}{2024-02-15} against the projects own checkstyle configuration to get all Java class files, that pass the own checkstyle test. We therefore used a tool from Maximilian Jungwirth\curl{https://github.com/sphrilix}{TODO} which is based on \textit{Styler}\curl{https://github.com/ASSERT-KTH/styler}{2024-03-11}~\cite{loriot2022styler}.
	From the Java classes that passed checkstyle we extract all methods that have a comment of any kind at the beginning of the method. This results in \numOriginal methods which we assume to be well readable.
	
	The fourth step is to generate poorly readable code from the well readable one. Therefore we use the proposed \rdh tool (\Cref{REDEC}). Afterwards we extract again all methods with a comment at the beginning of the method.
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	...
	First, we want to categorize the methods to make it easier to decide which methods to select. We also want to avoid over-representing very simple methods, such as Getters and Setters. Therefore, our first step is to apply stratified sampling~\cite{thompson2012sampling}. This allows us to divide the methods into different groups, so called strata, based on handcrafted features (\Cref{Conventional Calculation Approaches}).
...
%	There is no significant difference between the two, as a Mann-Whitney U test confirmed. The probability that the difference between \nonet and \texttt{original} is due to random variation is 92~\%. 
	...
	\begin{sloppypar}
	The violins of \autoref{fig:survey_ratings_violin_all} illustrate the distribution of the ratings in detail. Note that the evaluators were only able to rate the discrete values 1 to 5. The values of the violins in between are only for better visualization. The figure shows that for \texttt{original} and \texttt{just-pretty-print} most of the ratings are 3, 4 or 5. In addition, the violins of \texttt{merged} and \texttt{all7} look similar, with the latter having more 2 ratings. The violins of \texttt{newlines-many}, \texttt{rename} and \texttt{spaces-many} show more better and less worse ratings than the ones of \texttt{comments-remove}, \texttt{newlines-instead-of-space}, \texttt{newlines-few} and \texttt{tabs}.
	\end{sloppypar}
