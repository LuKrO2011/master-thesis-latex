...
\newcommand{\RDHs}{Readability Decreasing Modificatations\xspace}
...
%	In the domain of software development, the significance of code readability cannot be overstated. 
	...
	Methods in Java are syntactically the same, before and after applying \rdh. Functionality does not change either. However, if various modifications are applied many times, those modifications are capable of lowering the readability of source code, as \autoref{lst:example-method} shows.
...
%	% The towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} achieves an accuracy of 91.8\% when trained and evaluated on the new dataset. 
	...
	\begin{itemize}
		...
		\item We succeed in creating a tool that can automatically decrease the readability of Java class files (\Cref{Readability Decreasing Modifications}): \rdh.
...
%	You can find a visualization in \autoref{fig:dataset_generation}.
	...
	Our approach is divided into four parts (see \autoref{fig:dataset_generation}). The first three steps are used to mine well readable Java code. In a final step, we will modify the well readable code to achieve our second goal, namely poorly readable source code.
	...
	The fourth and final step is to generate poorly readable code from the well readable one. Therefore we use the proposed \rdh tool (see \Cref{Readability Decreasing Modifications}). Afterwards we again extract all methods with a comment at the beginning of the method.
...
\subsection{\RDHs} \label{Readability Decreasing Modifications}
	...
	\rdh initially converts the Java code of a well readable Java class file into an AST using the spoon library\curl{https://spoon.gforge.inria.fr/}{2024-15-02}~\cite{pawlak2016spoon}. In the end the AST is parsed back to Java code using the Pretty Printer of the spoon library\curl{https://spoon.gforge.inria.fr/}{2024-15-02}~\cite{pawlak2016spoon}. If nothing else is done, this results in \nonet. Note that the code of \nonet is slightly different from the original code, as the styling and formatting of the original code will be overwritten by the default formatting of the Pretty Printer.
	...
	\rdh performs one part of the \rdhs on the Abstract Syntax Tree (AST) representation of the Java files. It executes another part when pretty-printing the AST back into Java files. This second part cannot be displayed in the AST and is therefore executed at source code level immediately after the reverse transformation. In \autoref{tab:rdh-characteristics} we can see which modifications are applied when.
...
%	\end{table}
	...
	% Method extraction
	After \rdh applied the \rdhs to the Java files, we extract the methods. We require a method comment for all methods (see \Cref{Dataset Generation Approach}). We therefore use \texttt{removeComment} after completing the method extraction.
...
\subsection{Construction of Questionnaires} \label{Construction of Questionnaires}
	...
	% TODO: How was the initial dataset REDEC configs chosen?
	The first step was to find realistic configurations for the \rdh tool. After we created an initial dataset with some \mods, a pilot study was conducted and the \mods were adjusted based on the feedback. The final 9 configurations can be found in \autoref{tab:rdh_configurations}. Together with the original methods this resulted in 10 groups.
