...
%	In the domain of software development, the significance of code readability cannot be overstated. 
	...
	How can we manipulate code so that it is less readable afterwards? We introduce a tool called \RDHa. It uses a collection of heuristics (\RDHs) that, when applied to Java files, lower the readability of it. Such heuristics are replacing spaces with newlines or increasing the indentation of a code block by a tab or multiple spaces. Most changes also decrease readability when applied in reverse (replacing newlines with spaces, decreasing indentation).
	
	Methods in Java are syntactically the same, before and after applying \RDHsa. Functionality does not change either. However, if various modifications are applied many times, those changes are capable of lowering the readability of source code, as \autoref{lst:example-method} suggests.
...
%	% The towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} achieves an accuracy of 91.8\% when trained and evaluated on the new dataset. 
	...
	% Assumptions (short)
	We conducted a user study to validate the assumptions that the mined methods from the selected Java class files are well readable (\Cref{well-readable-assumption}) and that the \rdhs applied by \rdh can generate poorly readable code (\Cref{poorly-readable-assumption}). We thereby verify the generation approach of the mined-and-modified dataset.
	Additionally, we evaluate the performance of the towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} using the mined-and-modified dataset.
		
	% Contributions
	Our contributions are as follows:
	\begin{itemize}
		\item Although existing datasets~\citeolddataset have different structure we combine and unify them into one merged dataset (see \Cref{Work on Existing Datasets}).
		\item We argue that only part of the available data was used for training and evaluating the readability classification model of \citeauthor{mi2022towards}~\cite{mi2022towards} (see \Cref{Classification Considerations}).
		\item We develop an approach for mining not any but well readable Java methods (see \Cref{Dataset Generation Approach}).
		\item We achieve automated dataset generation for code readability and introduce a new dataset, namely the mined-and-modified dataset (see \Cref{Dataset Generation Approach}).
		\item We managed to create a tool that can automatically decrease the readability of Java class files (see \Cref{Readability Decreasing Heursitcs}).
		...
		\item We demonstrate that the towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} is limited (see \Cref{Readability Classification Model}).
		\item We show that our generation approach is working using a user study (see \Cref{Survey}).
		\item We show that our dataset can be used to train code readability classification models by training and evaluating the performance of the towards model of \citeauthor{mi2022towards}~\cite{mi2022towards} with and without the mined-and-modified dataset (see \Cref{Model training results}).
	\end{itemize}
...
\subsection{\RDHs} \label{Readability Decreasing Heursitcs}
	In this section we explain how we achieved to decrease the readability of code using \RDHa.
	...
	The \rdh tool initially converts the Java code of any well readable Java class file into an AST. In the end the AST is parsed back to Java code using an pretty printer. If nothing else is done, this results in \nonet. Note that the code produced by the tool in this way will be slightly different from the original input code, as the styling and formatting of the original code will be overwritten by the default formatting of the Java Pretty Printer of the spoon library\curl{https://spoon.gforge.inria.fr/}{2024-15-02}~\cite{pawlak2016spoon}.
...
%	\end{table}
	...
	\begin{listing}[tb]
		\begin{sublisting}{\linewidth}
			\begin{minted}[linenos, frame=lines, framesep=2mm]{Java}
				...
				*
				* @param number The input number to print the sign for.
				*/
	...
	\begin{table}[tb]
		\centering
		\caption{Available \RDHs along with when they are executed (on AST or Code), their configuration type (a array of probabilities or a single one) and whether they are included in the final dataset. See \Cref{appendix:rdh-config-file} for a concrete configuration.}
		\vspace{8pt}
		\label{tab:rdhs}
		\begin{tabular}{llllc}
			\toprule
...
%	For example, if the current tab count is 1 ($\backslash t \langle CODE \rangle$) (see \autoref{lst:example-method-well}, Line 7) a tab indentation results in a tab count of 2 ($\backslash t \backslash t \langle CODE \rangle $) (see \autoref{lst:example-method-well}, Line 8) and then a tab outdentation results in a tab count of 1 again ($ \backslash t \langle CODE \rangle$) (see \autoref{lst:example-method-well}, Line 9).
	For example:
	\begin{enumerate}
		\item The current tab count is 1 ($\backslash t \langle CODE \rangle$) (see \autoref{lst:example-method-well} Line 7)
		\item In the next line, we perform a tab indentation
		\item The current tab count is now 2 ($\backslash t \backslash t \langle CODE \rangle $) (see \autoref{lst:example-method-well}, Line 8)
		\item In the next line, we perform a tab outdentation
		\item The current tab count is now 1 ($\backslash t \langle CODE \rangle $) (see \autoref{lst:example-method-well}, Line 9)
	\end{enumerate}
